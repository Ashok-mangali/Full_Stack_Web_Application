...
const App = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get('http://127.0.0.1:5000/api/call'); //get the dada
  ...
  const handleSubmit = async (e, id) => {
    ...
    try {
      if (id) {
        ...
        await axios.put(`http://127.0.0.1:5000/api/up/${id}`, formData); // update the data
        ...
        await axios.post('http://127.0.0.1:5000/api/po', formData); //Adding the data 
        fetchData();
  ...
  const handleDelete = async (id) => {
    try {
      ...
      await axios.delete(`http://127.0.0.1:5000/api/del/${id}`); //Delete the data
